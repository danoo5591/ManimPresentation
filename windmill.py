...
class IntroduceIMO(Scene):
    ...
    def add_title(self):
        ...
        title.scale(1.25)
        logo = ImageMobject("img/iphone.png")
        logo.set_height(1)
    ...
    def get_flags(self):
        ...
        for country, count in countries_with_counts:
            ...
            try:
                image = ImageMobject("img/iphone.png")
                image.set_width(1)
                label = VGroup(*[
                    TextMobject(l)
...
class USProcessAlt(IntroduceIMO):
    ...
    def add_flag_and_label(self):
        flag = ImageMobject("img/iphone.png")
        flag.set_height(1)
        flag.to_corner(UL)
...
class Describe2011IMO(IntroduceIMO):
    ...
    def get_score_grid(self):
        ...
        flags = Group(*[
            ImageMobject("img/iphone.png").set_height(0.3)
            for row in data
        ])
        students = Group(*[
...
class ReadQuestions(Scene):
    def construct(self):
        background = ImageMobject("img/iphone.png")
...
class ContrastToOtherOlympiadProblems(AskWhatsOnTest):
    ...
    def get_problem_rects_2(self, page):
        pw = page.get_width()
        rects = VGroup(*[Rectangle() for x in range(3)])
        rects.set_stroke(width=2)
        rects.set_color_by_gradient([BLUE_E, BLUE_C, BLUE_D])
    ...
    def get_my_rectanlge(self, page):
        pw = page.get_width()
        rect = Rectangle()
        rect.set_stroke(width=2)
        rect.set_color_by_gradient(BLUE)
    ...
    def construct(self):
        self.my_zoom_to_other_questions()

    def my_zoom_to_other_questions(self):
        test = self.get_test2()
        rect = self.get_my_rectanlge(test)
        ...
        frame.generate_target()
        frame.target.scale(0.35)
        frame.target.move_to(rect)
        # big_rect = big_rects[1].copy()

        self.add(test[0], rect)
        self.play(
            FadeIn(big_rect),
...
class Test(ContrastToOtherOlympiadProblems):
    def construct(self):
        self.wait()
...
class TraditionalDifficulty(ContrastToOtherOlympiadProblems):
    def construct(self):
        test = self.get_test()
        rects = self.get_all_rects_2(test)
        for rect in rects:
            rect.reverse_points()
    ...
    def get_all_rects_2(self, test):
        rects = self.get_problem_rects_2(test[0])
        new_rects = VGroup(rects[1], rects[0], rects[2]).copy()
        new_rects[0].stretch(0.85, 1)
        new_rects[1].stretch(0.8, 1)
        new_rects[2].stretch(0.8, 1)
...
class TerryTaoQuote(Scene):
    def construct(self):
        ...
        tiny_tao = ImageMobject("img/iphone.png")
